...
function Exam06(){
    ...
    const addNumber = useCallback(()=>{
        // numbers.push(more); //쓰면 안되는 명령(원본을 직접 바꾸게됨)

        //concat 은 새로운 배열을 만들어내며, 이와 같이 신규 배열로 만들어 덮어쓰는게 '리액트'스러운 방법
        // setNumbers(numbers.concat(more));
        setNumbers([...numbers, more]);
    },[numbers, more]);

    const deleteNumber = useCallback(index=>{
        const choice = window.confirm("정말?");
    ...
    return(<>
            ...
            <div className="col">
                <ul className="list-group">
                    ...
                    <li className="list-group-item" key={index}>
                        ...
                        <FaXmark className="ms-4 text-danger" onClick={e=>deleteNumber(index)}/>
                    </li>
                     ))}
                </ul>
            </div>
        ...
        {/* 입력창과 버튼을 만들고 추가를 누르면 배열의 마지막 데이터 뒤에 추가되도록 구현 */}
