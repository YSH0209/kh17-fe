...
export default function AdminUsersScroll() {
    ...
    // const [last, setLast] = useState(true); //사용불가 (그렇게 됐다)

    const last = useRef(true); //연관항목 없이도 아무데서나 접근 가능한 동기식 데이터
    ...
    //throttling 을 통해서 성능 저하를 시켜야함


    // 문제 발생 : 
    //- 스크롤이벤트를 등록하는 시점의 sendMore 에는 lastAccountNo = null, size = 10 이다.
    //- 갱신이 스스로 안된다
    //- [1번 해결책] 사용되는 데이터를 Ref로 변경 (하책)
    //- [2번 해결책] 사용되는 함수를 Ref로 변경 (상책..?)

    const sednMoreRef = useRef(null);
    useEffect(() => {
        sednMoreRef.current = sendMore;
    }, [sendMore]);
                    ...
                    <tbody>
                        {list.map(account => (
                            <tr key={account.accountId}>
                                <td> <Link to={`/admin/usersDetail/${account.accountId}`}>
                                    {account.accountId}
                                </Link></td>
